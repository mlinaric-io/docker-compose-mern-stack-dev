...
if ! (( "$OSTYPE" == "gnu-linux" )); then
  echo "docker-compose-mern-stack-dev runs only on GNU/Linux operating system. Exiting..."
  exit
fi
...
start() {
  ...
  if [[ ! -d $PROJECT_NAME ]]; then	
    # generate initial commit in git
    rm -rf .git/
    git init
    git add .
...
{
    "name": "$PROJECT_NAME",
    "description": "docker-compose-mern-stack-dev project",
    "version": "1.0.0",
    "license": "MIT",
...
#JavaScript
  ...
  if [[ ! -d $PROJECT_NAME ]]; then
    docker-compose run node yarn add -D ynpx create-react-app
    docker-compose run node yarn create-react-app $PROJECT_NAME
  fi
